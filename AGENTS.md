...
- **Determinism.** Same seed + params must produce an identical run, tick for tick.
  ...
  `0x348e3c6eeec2e6d6`; only re-baseline deliberately, with a comment saying why.
...
- **Save format is versioned and never migrated.** Binary `SIMF` files, `VERSION = 3`
...
and confirm the value is still `0x348e3c6eeec2e6d6` unless the change deliberately
re-baselines it.
