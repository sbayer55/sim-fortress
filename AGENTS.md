...
- **Determinism.** Same seed + params must produce an identical run, tick for tick.
  ...
  `0x3bb742c2703aa248`; only re-baseline deliberately, with a comment saying why.
...
- **Save format is versioned and never migrated.** Binary `SIMF` files, `VERSION = 5`
...
and confirm the value is still `0x3bb742c2703aa248` unless the change deliberately
re-baselines it.
