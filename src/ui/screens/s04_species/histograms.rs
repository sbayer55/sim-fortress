...
pub(super) fn histograms(f: &mut Frame<'_>, area: Rect, sim: &Sim, id: SpeciesId) {
    ...
    let inner = panel::draw_with_hint(
        f,
        area,
        &format!("{}: trait distributions", sim.roster().display_name(id)),
        &format!("{} mean {} full {} half; hdr min mean max; 12 buckets, living", glyphs::CROSS, glyphs::FULL_BLOCK, glyphs::HALF_LOWER),
        panel::Kind::Outer,
    );
    // Thirteen traits fill a 3 x 5 grid of 25-column blocks (23-wide
    // histograms, two blocks spare). `block_h` is exactly the block height
    // (name, three histogram rows, axis, labels), and the grid starts on the
    // panel's first row: the fourth histogram row, the key line (now the
    // panel hint) and the old top spacer paid for the fifth grid row, so the
    // two comparison sections below still fit inside 41 rows.
    let col_w = 25u16;
    let block_h = 6u16;
    let rows = 5u16;
    for t in 0..Genome::LEN {
        ...
        let hist_area = Rect::new(x, y + 1, 24, 3);
        if s.count > 0 {
            bars::histogram(buf, hist_area, &s.hist[t], color, 2);
        }
        let axis: String = std::iter::repeat_n(glyphs::H_LINE, 24).collect();
        buf.set_stringn(x, y + 4, &axis, 24, theme::border());
        let mx = x + (crate::cast!((mean * 23.0).round() => u16)).min(23);
        buf.set_stringn(mx, y + 4, glyphs::CROSS.to_string(), 1, Style::default().fg(theme::TEXT_BRIGHT).bg(theme::PANEL_BG));
        buf.set_stringn(x, y + 5, "0", 1, theme::dim_text());
        buf.set_stringn(x + 23, y + 5, "1", 1, theme::dim_text());
        let n: u32 = s.hist[t].iter().map(|&v| u32::from(v)).sum();
        let peak_bucket = s.hist[t].iter().enumerate().max_by_key(|(_, v)| **v).map_or(0, |(i, _)| i);
        buf.set_stringn(x + 2, y + 5, format!("n={n}"), 7, theme::dim_text());
        buf.set_stringn(x + 11, y + 5, format!("mode .{:02}", crate::cast!(((crate::cast!(peak_bucket => f32) + 0.5) / 12.0 * 100.0).round() => u32)), 11, theme::dim_text());
    }
    // C7: the two trait-comparison sections moved here from the drift panel to
    // make room for its Disease section (the traits fill the top).
    let mut row = rows * block_h;
