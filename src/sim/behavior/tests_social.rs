...
fn alarm_spreads_to_social_kin_only() {
    ...
    idx.rebuild(&store, &w);
    mark_threats(&mut store, &idx, &w, 0, roster(), &PredationParams::default(), &sp);
    ...
    asocial.species = DEER;
    asocial.genome = genome(DEER);
...
fn cohesion_pulls_the_social_and_leaves_the_asocial_alone() {
    ...
    for (x, y) in [(12usize, 5usize), (8, 6)] {
        ...
        k.species = DEER;
        k.genome = genome(DEER);
        kin_ids.push(store.insert(k));
    }
    let mut focal = test_creature(50, 5);
    focal.species = DEER;
    focal.genome = genome(DEER);
    focal.genome.0[IDX_SOCIALITY] = 0.98;
    let view = TickView::build(&store, &t, &w, roster(), &gp, &DiseaseParams::default());
...
fn pack_joins_the_shared_target() {
    ...
    let mut idx = SpatialIndex::new(&w);
    idx.rebuild(&store, &w);
    let view = TickView::build(&store, &t, &w, roster(), &GeneticsParams::default(), &DiseaseParams::default());
...
fn pack_shares_the_kill() {
    ...
    assert_eq!(tallies.hunt_kills[WOLF.index()], 1);
}
