...
fn wary_is_retained_within_the_release_radius() {
    ...
    prey.species = HARE;
    prey.genome = genome(HARE);
    prey.goal = Goal::Wary;
    prey.wary_until = 10;
    prey.wary_by = Some((20, 5, WOLF));
    ...
    mark_threats(&mut store, &idx, &w, 1, roster(), &pp, &sp);
...
fn day_flush_is_one_event_per_region() {
    ...
    tallies.wary_today.insert((0, VOLE, WOLF), 5);
    tallies.wary_today.insert((0, HARE, WOLF), 3);
    tallies.wary_today.insert((1, DEER, LYNX), 2);
    flush_wary(&w, &mut events, &time, roster(), &mut tallies);
    ...
    assert_eq!(first.species, Some(VOLE), "the dominant prey names the event");
...
fn wary_tally_resets_at_the_next_day() {
    let mut tallies = DeathTallies::new(N_SPECIES);
    tallies.wary_today.insert((0, VOLE, WOLF), 1);
