...
fn asocial_creatures_are_always_alone() {
    ...
    let hare = crate::sim::SpeciesId::Hare;
...
fn histogram_counts_every_creature_exactly_once() {
    ...
    let g = group_census(&store, &SocialParams::default());
    let c = crate::sim::stats::census(&store);
    for i in 0..6 {
...
fn sim_refreshes_group_stats_and_survives_a_save() {
    ...
    assert_eq!(sim.group_stats, group_census(&sim.creatures, &sim.params.social));
    ...
    assert_eq!(loaded.group_stats, group_census(&loaded.creatures, &loaded.params.social));
